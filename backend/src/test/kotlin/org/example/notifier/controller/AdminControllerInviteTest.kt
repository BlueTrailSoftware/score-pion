...
class AdminControllerInviteTest {
    ...
    fun setup() {
        ...
        getAllAdminsUseCase = mock(GetAllAdminsUseCase::class.java)
        updateAdminActiveStatusUseCase = mock(UpdateAdminActiveStatusUseCase::class.java)
        getPendingInvitationsUseCase = mock(GetPendingInvitationsUseCase::class.java)
    ...
    fun `inviteAdmin should return success when use case succeeds`() = runBlocking {
        ...
        assertEquals("Admin invitation sent successfully", response.body?.message)
        assertEquals("inv-1", response.body?.data?.id)
    }
    ...
    fun `inviteAdmin should return badRequest on IllegalArgumentException`() = runBlocking {
        ...
        assertEquals("An invitation for this email already exists", response.body?.message)
    }
    ...
    fun `inviteRecruiter should return success when use case succeeds`() = runBlocking {
        ...
        assertEquals(HttpStatus.OK, response.statusCode)
        assertEquals("success", response.body?.status)
        assertEquals("Invitation sent successfully", response.body?.message)
    }
