...
class UpdateEmailTest {
    ...
    @Test
    fun `updateEmail should return 200 with updated recipients on success`() = runBlocking {
        whenever(securityUtils.getCurrentUserId()).thenReturn(adminId)
        val command = UpdateGlobalRecipientEmailCommand("old@example.com", "new@example.com", adminId)
        whenever(updateGlobalRecipientEmailUseCase.execute(command)).thenReturn(updatedResult)

        val response = controller.updateEmail(UpdateEmailRequest("old@example.com", "new@example.com"))

        assertEquals(HttpStatus.OK, response.statusCode)
        assertEquals("success", response.body?.status)
        assertEquals("Email updated successfully", response.body?.message)
        assertTrue(response.body?.data?.emails?.contains("new@example.com") == true)
    }

    @Test
    fun `updateEmail should return 400 on IllegalArgumentException`() = runBlocking {
        whenever(securityUtils.getCurrentUserId()).thenReturn(adminId)
        val command = UpdateGlobalRecipientEmailCommand("missing@example.com", "new@example.com", adminId)
        whenever(updateGlobalRecipientEmailUseCase.execute(command))
            .thenThrow(IllegalArgumentException("Email not found"))

        val response = controller.updateEmail(UpdateEmailRequest("missing@example.com", "new@example.com"))
    ...
    @Test
    fun `updateEmail should return 500 on unexpected exception`() = runBlocking {
        whenever(securityUtils.getCurrentUserId()).thenReturn(adminId)
        val command = UpdateGlobalRecipientEmailCommand("old@example.com", "new@example.com", adminId)
        whenever(updateGlobalRecipientEmailUseCase.execute(command)).thenThrow(RuntimeException("DB error"))

        val response = controller.updateEmail(UpdateEmailRequest("old@example.com", "new@example.com"))

        assertEquals(HttpStatus.INTERNAL_SERVER_ERROR, response.statusCode)
        assertEquals("error", response.body?.status)
        assertTrue(response.body?.message?.contains("Failed to update email") == true)
    }
}