...
@Component
class UpdatePositionActiveStatusUseCase(
    private val openPositionService: OpenPositionService
) {

    suspend fun execute(command: UpdatePositionActiveStatusCommand): PositionResult? {
        val updated = openPositionService.updatePositionActiveStatus(command.positionId, command.isActive)
